---
title: "Session One: Getting to Know R and RStudio"
...
Blocked content (with grey or white background and grey border) are called **R Chucks** which are R codes written you would write a script. The **#** symbol indicates a comment usually used to give information about some line of code and **##** indicates results or output of code which has been `rendered`.
...
R as an implementation of the S programming language has much of the S code but with subtle difference, like [lexical scoping](../Level Two/#Lexical Scoping) which we shall discuss in detail in level two. 
...
When downloading is complete, click on the executable program which should be at the status bar or your downloads folder, then click run. This should take you through the following installation windows:
  ...
  * Next, you will need to indicate additional tasks to be performed during setup like creating a desktop icon, quick launch and registry files. If you will be using R often, you can click on the desktop or the quick launch options, but make sure to check the [registry](../../Level Two/#Registry) option as it  is vital in R; and with that, installation should begin. 

I am currently working on a quick instructional [demo](../../Slides/Demo/1.Downloading and Installing R.html) on how to install R on a windows operating system, please keep checking the demo page.
...
Okay, now let's practically apply these concepts by installing two packages. The first is a [web scrapping](../../Programming for Non-programmers/Other useful terms.html#web.scrapping) package known as `rvest` and the other is a graphing package known as `ggplot2`. Let's assume that we need `rvest` for only one session, but we will use `ggplot2` frequently to plot graphs. Therefore, we will install `rvest` in a temporary directory and `ggplot2` in the newly created library. 
...
It would be useful for you to note that some packages come with other packages  referred to as [Imports](../../Programming for Non-programmers/Other useful terms.html#Imports.Packages). There are also those packages that package [depends](../../Programming for Non-programmers/Other useful terms.html#Dependecies) on and other packages it [suggests](../../Programming for Non-programmers/Other useful terms.html#Suggests). Always read the package's documentation using `help(package = "name")`to know more about the package. An example is `help(package = "rvest", lib.loc = temp())`.
...
First establish the full [path](../../Programming for Non-programmers/Other useful terms.html#path) to your working directory. Then input that path to the `setwd()` (set working directory) function.
...
1. What is your understanding of the following terms
    ...
    a. Message
1. What are packages 
1. How can you determine the installed packages in any library?
1. Which of the pre-installed packages are on the search path?
