# Web Scraper
webScraper <- function(url, css, ..., obj = NULL, constructTable = FALSE, withOutTags = FALSE) {
   if (!is.null(obj)) {
      url <- NULL
   } else {
      obj <- readLines(url, ...)
   ...
   if (m) {
      simpleSelectors(css, obj, constructTable, withOutTags)
   }
}
...
simpleSelectors <-  function(css, doc, asis = TRUE, content = TRUE, constructTable = FALSE, withOutTags = FALSE, encoding = "UTF-8") {
   ...
   if (withPseudo) {
      nth <- sub("[^(]+\\(([^)]+)\\)", "\\1", pseudoClass)
      rows <- nthInterpreter(nth = nth, nDoc = nrow(clsTagMat))
      clsTagMat <- clsTagMat[rows,, drop = FALSE]
   }
